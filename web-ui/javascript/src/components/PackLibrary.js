...
class PackLibrary extends React.Component {
    ...
    render() {
        ...
        // Set of pack UUIDs already present on the connected device, used to grey out matching entries in the local library
        const deviceUuids = new Set(this.state.device.packs.map(pack => pack.uuid));
        // Set of pack UUIDs already present in the local library, used to grey out matching entries on the device
        const libraryUuids = new Set(this.state.library.packs.map(group => group.uuid));
        return (
                    ...
                    <div className={`device-dropzone ${this.state.dragging !== null ? 'highlighted-dropzone' : ''}`}
                        ...
                        {this.state.device.packs.length > 0 && <div className="pack-grid">
                            {this.state.device.packs.map((pack,idx) =>
                                <div key={pack.uuid}
                                     title={libraryUuids.has(pack.uuid) ? t('library.device.alreadyInLibrary') : undefined}
                                     draggable={true}
                                     className={`pack-tile pack-${pack.format} pack-draggable ${pack.nightModeAvailable && 'pack-night-mode'} ${libraryUuids.has(pack.uuid) ? 'pack-in-library' : ''}`}
                                     onDragStart={event => {
                    ...
                    <div className={`library-dropzone ${this.state.dragging === 'device-pack' ? 'highlighted-dropzone' : ''}`}
                        ...
                        {this.state.library.packs.length > 0 && <div className="pack-grid">
                            {this.state.library.packs.map(group =>
                                <div key={group.uuid}
                                     title={deviceUuids.has(group.uuid) ? `${group.uuid}\n${t('library.local.alreadyOnDevice')}` : group.uuid}
                                     draggable={this.isPackDraggable(group.packs[0])}
                                     className={`pack-tile ${this.isPackDraggable(group.packs[0]) ? 'pack-draggable' : 'pack-not-draggable'} ${group.packs[0].nightModeAvailable && 'pack-night-mode'} ${deviceUuids.has(group.uuid) ? 'pack-on-device' : ''}`}
                                     onDragStart={event => {
                                         // Drag first pack
